...
public class JsonNodeJSObject
    ...
    public Object getMember(String name) {
        return switch (name) {
            ...
            case "toJSON" -> new JavaScriptMethodCall(args -> jsonNode.toPrettyString());

            case "length" -> jsonNode instanceof ArrayNode arrayNode ? arrayNode.size() : 0;

            default -> wrap(jsonNode.get(name));
        };
    }
